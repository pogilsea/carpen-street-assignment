...
import {ConditionType, JoinType, QueryBuilder, UpdateQuerySet} from '@lib/mysql/query-builder';
import {HttpStatus} from '@lib/http/status-code';
import {ErrorCode} from '@lib/http/error-code';
import {MySQLConnection} from '@lib/mysql/mysql-connector';
...
export class MySQLWrapper implements IMySQLWrapper {
    ...
    protected asserCondition(conditions: ConditionType[], opt?: GetQueryPropertyType) {
        if (conditions.length < 1 && (!opt || !opt.noCondition)) {
            throw createError(HttpStatus.INTERNAL_SERVER_ERROR, 'Bad Request', {
                errorCode: ErrorCode.NO_SELECT_CONDITION,

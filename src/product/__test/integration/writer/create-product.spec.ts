...
describe('[Integration] 작가 상품 정보 추가', () => {
    let uri = '/products';
    describe('상품 변경: SUCCESS', () => {
        let productId = 0;
        it('상품 정보 추가시, HTTP Status 200 with OK', async () => {
            const writerToken =
    ...
    describe('상품 추가: ERROR', () => {
        ...
        describe('[Forbidden] 인증되지 않은 Role', () => {
            it('에디터가 상품을 추가하려고 할때, HTTP Status 403을 반환', async () => {
                const editorToken =
        ...
        describe('[Bad Request] 필수값 누락', () => {
            const writerToken =
                'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6Miwicm9sZSI6IldSSVRFUiIsImlhdCI6MTY1Nzg1NDk3OSwiZXhwIjoxNjU5NTk5MjM0MDc5fQ.HghFqZiBpw9JtkMj-0kACQJfRmBfpsYiLrYF3-eUMiM';
            const Authorization = `Bearer ${writerToken}`;
            it('가격 필드 누락, HTTP Status 400을 반환', async () => {
            ...
            it('제목 필드 누락, HTTP Status 400을 반환', async () => {
                const param = {content, price};
                const {status, body} = await testControl.request.post(uri, param, {Authorization});
                const errorMessage = body.error.errorMessage;
                chai.assert.equal(status, 400);
                chai.assert.include(errorMessage, `required`);
            ...
            it('상품 내용 필드 누락, HTTP Status 400을 반환', async () => {
                const param = {title, price};
                const {status, body} = await testControl.request.post(uri, param, {Authorization});
                const errorMessage = body.error.errorMessage;
                chai.assert.equal(status, 400);
                chai.assert.include(errorMessage, `required`);
